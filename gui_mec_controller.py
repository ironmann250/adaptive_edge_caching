...
class CollaborativeCache:
    ...
    def __getfile(self, mec, content_hash):
        ...
        try:
            urllib.request.urlretrieve(request_link, filename)
...
class LocalCache:
    ...
    def push(self, page, precache=0):
        ...
        if new_node.id in self.table:
            ...
            if reply:
                ...
                # decision is right | send add cache only when length > cache_size
    ...
    def plot_association_accuracy(self, ax):
        explode = []
        val = [self.matches.right, self.matches.wrong]
        for i in val:
            if i == max(val):
                explode.append(0.1)
            else:
...
class RuleStore:
    ...
    def rule_list(self):
        r_list = []
        for j in range(len(self.lru_list)-1, -1, -1):
            r_list.append((self.lru_list[j], self.rules[len(self.lru_list[j])][self.lru_list[j]]))
            if len(r_list) >= 6:
                break
        return r_list
...
def plot_graphs():
    ...
    # plot asso
    store.plot_association(ax=axis[0])
    # plot asso acuracy
    store.plot_association_accuracy(ax=axis[1])
    # plot_rtts()
    store.delay.plot_data(ax=axis[2])
    # plot_cpu()
    cpu_record.add_data()
    ...
    # plot hit ratio
    store.plot_cache_performance(ax=axis[5])
